...
using System.Linq;
using nickeltin.PathCreation.Utility;
using UnityEngine;
...
namespace nickeltin.PathCreation
{
    ...
    public class BezierPath
    {
        ...
        public BezierPath(IEnumerable<Vector2> points, PathSpace space = PathSpace.xyz, bool isClosed = false) : this(
            points.Select(p => new Vector3(p.x, p.y)), isClosed, space)
        {
        }
        ...
        {
            if (space == PathSpace.xy) pointPos.z = 0;
            ...
            else
            {
                ...
                if (space != PathSpace.xyz)
                    for (var i = 0; i < NumPoints; i++) // from xz to xy
                        if (space == PathSpace.xy) points[i] = new Vector3(points[i].x, points[i].z, 0);
                        // from xy to xz
            ...
            if (isClosed)
            {
                ...
                if (controlMode != ControlMode.Mirrored && controlMode != ControlMode.Automatic)
                {
                    ...
                    lastAnchorSecondControl = points[points.Count - 1] +
                                              (points[points.Count - 1] - points[points.Count - 2]).normalized * (dstBetweenStartAndEndAnchors * .5f);
